...
// Описанием типа объекта мы можем ограничить его свойства и указать опциональные (необязательные) свойства
type MyObj = {
  d:string,
  f?:string, // опциональное свойство (prop?)
}
...
// Получаем первую ссылку в документе по id, через QS. Сразу указываем, что это объект, представляющий ссылку
const HrefEl = document.getElementById('href') as HTMLAnchorElement
const HrefEl2 = document.querySelector('a') as HTMLAnchorElement

// Получаем все ссылки в документе через QS. Сразу указываем, что это список нод, объектов ссылка
const AllHrefEls = document.querySelectorAll('a') as NodeListOf<HTMLAnchorElement>
